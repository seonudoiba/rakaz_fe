...
export const inventoryApi = {
  getTankMonitoring: async (stationId: string) => {
    try {
      ...
      return await apiClient.get<{ tanks: Tank[] }>(
        `/inventory/tanks/${stationId}`,
      );
    } catch (error: any) {
      // If 404, return empty tanks array instead of throwing
      if (error.response?.status === 404) {
        console.warn(
          `Tank monitoring endpoint not found for station ${stationId}, returning empty array`,
        );
        return { tanks: [] };
      }
      throw error;
  ...
  createTank: async (data: {
    ...
    return apiClient.post("/inventory/tanks", data);
  },
  ...
  updateTank: async (
    ...
    return apiClient.put(`/inventory/tanks/${id}`, data);
  },

  deleteTank: async (id: string) => {
    return apiClient.delete(`/inventory/tanks/${id}`);
  },
  ...
  updateTankLevel: async (
    id: string,
    data: { currentLevel: number; percentage: number },
  ) => {
  ...
  getInventoryLogs: async (
    ...
    return apiClient.get<InventoryLog[]>(
      `/inventory/logs/${stationId}?${query.toString()}`,
    );
  },

  getProductMovement: async (
    stationId: string,
    params?: { productType?: string; days?: number },
  ) => {
    const query = new URLSearchParams();
    if (params?.productType) query.append("productType", params.productType);
    if (params?.days) query.append("days", String(params.days));
    return apiClient.get(
      `/inventory/movement/${stationId}?${query.toString()}`,
    );
  },
  ...
  getProductPrices: async (stationId?: string) => {
    const url = stationId
      ? `/inventory/prices?stationId=${stationId}`
      : "/inventory/prices";
    return apiClient.get<
      Array<{
        id?: string;
