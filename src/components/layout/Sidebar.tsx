...
const Sidebar: React.FC<SidebarProps> = ({ open, setOpen }) => {
  ...
  const getFilteredMenu = () => {
    ...
    switch (user.role) {
      ...
      case 'SUPERVISOR':
        return menuItems.filter(item => 
          ['Dashboard', 'Sales', 'Pumps', 'Inventory', 'Expenses', 'Employees', 'Reports'].includes(item.label)
        );
      case 'ATTENDANT':
        return menuItems.filter(item => 
          ['Sales'].includes(item.label)
        );
      // case 'DEPOT_MANAGER':
      //   return menuItems.filter(item => 
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-800">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded bg-petroleum-seagreen flex items-center justify-center">
              <span className="text-sm font-bold text-petroleum-dark">R</span>
            </div>
            {open && (
              <span className="text-lg font-bold text-white">Rekaz</span>
            )}
          </div>
          <button 
